...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    public class ARKitBlendShapeVisualizer : MonoBehaviour
    {
        ...
        void CreateFeatureBlendMapping()
        {
            ...
            leftEyeIndex = skinnedMeshRenderer.sharedMesh.GetBlendShapeIndex(strPrefix + "eyeBlink_L");
            ...
            rightEyeIndex = skinnedMeshRenderer.sharedMesh.GetBlendShapeIndex(strPrefix + "eyeBlink_R");
            ...
            using (var blendShapes = m_ARKitFaceSubsystem.GetBlendShapeCoefficients(m_Face.trackableId, Allocator.Temp))
            {
                foreach (var featureCoefficient in blendShapes)
                {
                    ...
                    if (m_FaceArkitBlendShapeIndexMap.TryGetValue(featureCoefficient.blendShapeLocation, out mappedBlendShapeIndex))
                    {
                        ...
                        {
                            var value = featureCoefficient.coefficient * coefficientScale;
                            EffectIfNeed(mappedBlendShapeIndex, value);
                            skinnedMeshRenderer.SetBlendShapeWeight(mappedBlendShapeIndex, value);
                        }
                    }
                }
            }
        ...
        void EffectIfNeed(int index, float value)
        {
            if (index == leftEyeIndex)
            {
                var strength = 0.000001f * (100 - value) * (100 - value);
                leftEye.localScale = new Vector3(1, 1, strength);
            }

            if (index == rightEyeIndex)
            {
                var strength = 0.000001f * (100 - value) * (100 - value);
                rightEye.localScale = new Vector3(1, 1, strength);
            }
        }
    }
}